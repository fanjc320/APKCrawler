...
class XiaomiSpider(scrapy.Spider):
    ...
    def parse_diff_category(self, response: scrapy.http.Response):
        ...
        """
        print("parse_diff_category >>>>>>>>>>>>>>>>>>>>>");

        category_urls = response.css(".sidebar ul.category-list li a::attr(href)").getall()
        print("parse_diff_category >>>>>>>>>>>>>>>>>>>>> len(category_urls):", len(category_urls));
        for category_url in category_urls:
            category_id = os.path.basename(category_url)
            print("xiaomi_spider >>>>>>>>>>>>>>  category_url:", category_url, " category_id:", category_id)
            json_url = "http://app.mi.com/categotyAllListApi?page=0&categoryId={}".format(category_id)
            print("parse_diff_category category_url:"+category_url)
            print("parse_diff_category json_url:"+json_url)
    ...
    def parse_category_list(self, response: scrapy.http.Response):
        ...
        parse category list.
        """
        print("parse_category_list response status:", response.status, " response body:", response.body)
        return
